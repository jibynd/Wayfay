...
class Wayfay:
    ...
    def init_data(self, y, X):
        ...
        #self.data["KX"] = Data("KX", np.diag(np.diag(dot(X.T, X))), "both") 
...
class Wishart(RandomVariable):
    ...
    @property
    def scale(self):
        B, ld, sig = _vars["B"].get(), _vars["ld"].get(), _vars["sig"].get() 
    ...
    @property
    def df(self):
        d = _data["X'X"].get().shape[0] #GEtfrom Z size"
        return d + _meta["d0"]
